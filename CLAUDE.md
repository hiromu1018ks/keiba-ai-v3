...
**すべての応答・説明・コメント・コミットメッセージは必ず日本語で行うこと。** 技術用語とコード識別子は原文のままでよい。この指示は他のどの指示よりも優先する。
...
**Core Value:** オッズ非依存の確率 `p_fukusho_hit` と固定オッズ時点のEVで、過小評価されている馬の複勝払戻対象入り可能性を**リークなく**検出し、race_id単位・時系列順の再現可能なバックテストでその有効性を定量評価できること。それ以外がすべて失敗しても、この再現性とリーク防止だけは守らなければならない。
