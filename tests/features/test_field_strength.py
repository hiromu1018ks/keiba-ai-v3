...
# WR-04 (10-08 gap-closure): 本番規模により近い 1000 race smoke・200 race では発覚しない OOM リスクを検出。
# 既存の PROD_SMOKE_N_RACES=200 とは別系統（既存テストは温存）。W-3 聖域（per-race 予算 ≤0.30s/race +
# 準二次スケーリングガード・cProfile 上位3位に Python ループ無し）を破らない。本定数を使うテストは
# test_production_scale_smoke_large・KEIBA_RUN_PERF_TESTS=1 でのみ実行（default skip）。
# ※ 本定数は「閾値緩和」でなく「検出力向上」（200 race では発覚しない OOM リスクを 1000 race で検出）。
PROD_SMOKE_N_RACES_LARGE: int = 1000
...
def test_production_scale_smoke_no_h_squared_blowup() -> None:
    ...
    WR-05 (10-08 gap-closure): peak memory 主指標を rss_gb (getrusage) に変更。
    tracemalloc は Python ヒープのみを追跡し・pandas/numpy の native buffer (C レベル malloc) を含まない
    ため大幅に過小評価される。rss_gb はプロセス全体の peak RSS（他テストの peak を含む可能性はあるが・
    pandas DataFrame の native buffer を捕捉できる）。tracemalloc は参考記録扱い。
    ...
    # WR-05 (10-08 gap-closure): rss_gb を主指標に変更（tracemalloc は numpy/pandas native buffer
    # 含まず過小評価・max(peak_py_gb, rss_gb) でなく rss_gb のみで判定）。
    peak_gb = rss_gb
    logger_perf = logging.getLogger("test_field_strength.perf")
    logger_perf.info(
        "PROD smoke: peak_rss=%.2fGB (py_heap=%.2fGB・参考・tracemalloc は Python ヒープのみで numpy/pandas "
        "native buffer 含まず過小評価)",
        rss_gb, peak_py_gb,
    )
    assert peak_gb <= PROD_PEAK_MEM_BUDGET_GB, (
        f"PROD_PEAK_MEM_BUDGET_GB 違反: peak_rss={peak_gb:.2f}GB "
        f"(py_heap={peak_py_gb:.2f}GB・参考) > {PROD_PEAK_MEM_BUDGET_GB}GB・"
...
# ---------------------------------------------------------------------------
# WR-04 (10-08 gap-closure): 本番規模 1000 race smoke・200 race では発覚しない OOM リスクを検出
# ---------------------------------------------------------------------------
@pytest.mark.skipif(
    not os.environ.get("KEIBA_RUN_PERF_TESTS"),
    reason=(
        "本番規模 1000 race smoke は時間を要するため default skip。"
        "KEIBA_RUN_PERF_TESTS=1 で明示的実行。WR-04・200 race では発覚しない OOM リスクの検出力向上。"
    ),
)
def test_production_scale_smoke_large() -> None:
    """WR-04 (10-08 gap-closure): 本番規模 1000 race smoke・200 race (PROD_SMOKE_N_RACES) では
    発覚しない OOM リスクを検出する（W-3 聖域遵守・緩和でなく検出力向上）.

    既存の test_production_scale_smoke_no_h_squared_blowup (200 race) と同構造だが・1000 race
    (PROD_SMOKE_N_RACES_LARGE) で compute_field_strength_profile を実行し・
    peak_rss ≤ PROD_PEAK_MEM_BUDGET_GB(8.0) で PASS する。

    **W-3 聖域（後追い緩和禁止）**: 本テストは「閾値緩和」でなく「検出力向上」である。200 race では
    発覚しない OOM リスク（SOURCE_RACE_BATCH_SIZE=100 でも source race 数が増えるとバッチ数が増え・
    各バッチの materialize が累積して RSS が膨らむ可能性）を 1000 race で検出する。W-3 性能聖域
    （per-race 予算 ≤0.30s/race・準二次スケーリングガード・cProfile 上位3位に Python ループ無し）は
    test_compute_field_strength_profile_performance / test_no_python_loop_hot_spot が別途保証する。
    """
    import resource
    import time
    import tracemalloc
    ...
    raw_history = _build_perf_history(
        PROD_SMOKE_N_RACES_LARGE,
        PROD_SMOKE_HORSES_PER_RACE,
        PROD_SMOKE_PAST_RUNS,
    )
    n_rows = len(raw_history)
    # 1000 race × 14 × 7 = ~98000 行規模であることを確認（テスト前提）
    assert n_rows >= 50_000, (
        f"production-scale smoke (large) の行数が小さすぎる: {n_rows}・"
        f"1000 race 規模の OOM リスク検出に十分な規模でない（テスト前提違反）"
    )
    ...
    logger_large = logging.getLogger("test_field_strength.perf_large")
    logger_large.info(
        "PROD smoke (large・1000 race): peak_rss=%.2fGB (py_heap=%.2fGB・参考) elapsed=%.1fs n_rows=%d",
        rss_gb, peak_py_gb, elapsed, n_rows,
    )

    # rss_gb を主指標に（WR-05・tracemalloc は numpy/pandas native buffer 含まず過小評価）
    assert rss_gb <= PROD_PEAK_MEM_BUDGET_GB, (
        f"PROD_PEAK_MEM_BUDGET_GB 違反 (large・1000 race): peak_rss={rss_gb:.2f}GB "
        f"(py_heap={peak_py_gb:.2f}GB・参考) > {PROD_PEAK_MEM_BUDGET_GB}GB・"
        f"H² 積 materialize / バッチ累積メモリの疑い・PLAN 01 per-source-race バッチ構造を確認すること。"
    )
    assert len(result) == n_rows, (
        f"入力 {n_rows} 行に対して出力 {len(result)} 行・行数不一致（large smoke 検出）"
    )
