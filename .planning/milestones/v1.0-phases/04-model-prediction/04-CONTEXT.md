...
- **`model_version` 採番方式** — `feature_snapshot_id` と整合する方式・Cycle 3 NEW-4 で正準形式を `{feature_snapshot_id}-{model_type_short}-v{N}` に統一（例: `20260620-1a-postreview-v2-lgb-v1` / `20260620-1a-postreview-v2-cb-v1`・feature_snapshot_id 全体を prefix）。両モデルを区別する `model_type`（lightgbm/catboost）と組み合わせ
