...
cycles:
  ...
  - cycle: 4
    reviewer: adversarial re-review（user-authorized beyond MAX_CYCLES=3・Cycle 3 actionable 4件の PLAN/doc 反映を独立検証 + cycle-1/2 解決 HIGH の回帰走査 + 新規懸念の対抗走査）
    reviewed_commit: 9eeb907
    reviewed_at: 2026-06-20
    status: FULLY CONVERGED — Cycle 3 の4件（NEW-M1 MEDIUM・NEW-4 残渣 LOW・NEW-L1 LOW・NEW-L2 LOW）を全て PLAN/doc の実行可能契約に変換済みを検証・cycle-1/2 解決 HIGH の回帰なし・新規 HIGH なし・actionable=0 達成を確認
status: CONVERGED — Cycle 4 検証完了・残存 HIGH 0件・actionable 0件・execute-phase 進行可能
...
<!-- ============================================================ -->
<!-- CYCLE 4 — 第3再計画（commit 9eeb907）に対する adversarial re-review -->
<!-- Cycle 1-3 セクションは上記に監査証跡として保全 -->
<!-- ============================================================ -->

### Cycle 4 — 第3再計画（commit 9eeb907）に対する独立検証（adversarial re-review）

**実施日**: 2026-06-20
**対象 commit**: 9eeb907 "resolve Cycle 3 actionable 4件 (NEW-M1/NEW-4/NEW-L1/NEW-L2)"
**レビュア**: adversarial re-review（user-authorized beyond MAX_CYCLES=3）
**判定**: **FULLY CONVERGED — Cycle 3 の4件は全て実行可能契約に変換済みを検証・cycle-1/2 解決 HIGH の回帰なし・新規 HIGH/actionable なし・actionable=0 達成。**

### Cycle 4 の方法

6 PLAN.md + 04-CONTEXT/PATTERNS/RESEARCH の改訂箇所を PLAN 単位で照合し、3 任務を実行: (A) Cycle 3 の4件 actionable が `/gsd-execute-phase` が実装する実行可能契約（action / acceptance / test / threat_model / must_haves / Artifacts / docstring）に変換されたか、(B) Cycle 1/2 で解決済の 19 HIGH + NEW HIGH-1 + NEW-2/3/5 + residual #13 が第3再計画で回帰していないか、(C) 対抗的に新規懸念を走査（np.allclose vs np.array_equal の整合・DDL 列数表記揺れ・sklearn 私有 API の実装可能性・pred_proba / as_of_datetime / scoping の連動・CatBoost `_code` cat_features の学習+予測両経路・label join 境界の二重防御）。各判定は grep / 行番号 excerpt で裏付け。

### 任務 A — Cycle 3 の4件の収束検証

| Cycle 3 指摘 | 判定 | 検証根拠（commit 9eeb907 で変更された PLAN/doc の箇所） |
|---|---|---|
| **NEW-M1 (MEDIUM)**: `load_native_artifact` step (c) の sklearn 1.9 私有 API 依存 | **FULLY RESOLVED** | 04-02 PLAN.md の4箇所で実行可能契約化: (1) must_haves truths 行30 に「scikit-learn==1.9.0 pin を安定性保証・固定許容誤差 rtol=1e-12, atol=1e-12 検証」明記・(2) behavior test `test_artifact_save_load_roundtrip`（行165）に `np.allclose(saved_proba, loaded_proba, rtol=1e-12, atol=1e-12)` assert 追加・(3) action step 5(c)（行190）に「pin を安定性保証として扱い・pin 破壊で即時 RED で検出・将来 upgrade 時の roundtrip 再検証手順」明記・(4) acceptance（行212）/ threat T-04-11（行240）/ Artifacts（行268）に全て反映。**pyproject.toml 行16 に `scikit-learn==1.9.0` pin が実在することを確認**（pin が存在しないと安定性保証が宙に浮く懸念を排除）。pin 破壊 = roundtrip test の即時 RED で検出という閉じた契約。 |
| **NEW-4 残渣 (LOW)**: 3ファイルの古い model_version 形式 | **FULLY RESOLVED** | 04-01-PLAN.md 行179（`test_model_version_numbering` stub docstring）・04-PATTERNS.md 行157・04-CONTEXT.md 行50 の3箇所を `20260620-1a-postreview-v2-lgb-v1` / `-cb-v1` の正準形式に統一済みを確認。実装者が古い短縮形式（`20260620-1a-lgb-v1`）を権威参照する経路を全て閉塞。action/acceptance/test 中心部は既に Cycle 2 で正準形式固定済みであり・Cycle 4 は残渣掃除完了。 |
| **NEW-L1 (LOW)**: `calibrator.joblib 一時退避で再構築可能` が物理的不可能 | **FULLY RESOLVED** | 04-02 PLAN.md の契約を「物理的に不可能な fallback を残さない」方向に全面書き直し: (1) behavior test（行165）を「base + calibrator.joblib の両方が揃った状態での roundtrip」のみを契約とし・calibrator.joblib 欠落時 `load_native_artifact` が FileNotFoundError で fail-loud することを別 assert で検証に変更・(2) action step 5(c)・docstring（行191/193）に「calibrator.joblib は必須・native base 単独では isotonic 閾値/sigmoid 係数なしで不可・手動復元は calibrator 数値 payload が別途保存されている場合のみ」と明記・(3) acceptance（行213）/ threat T-04-11 / must_haves / Artifacts に全て反映。物理的に不可能な主張を契約から完全除去。 |
| **NEW-L2 (LOW)**: RESEARCH D-12 `calib_method='none'` と 04-01 DDL CHECK の不整合 | **FULLY RESOLVED** | 04-RESEARCH.md D-12 行718 の comment から `'none'` を除去し・04-01 DDL CHECK（`calib_method IN ('isotonic','sigmoid')`・行114/133/274）と整合することを確認。Phase 4 の LightGBM/CatBoost は両方必ずキャリブレーションする（`calibrate_model` が isotonic/sigmoid を強制）ので実行時影響なし・将来未キャリブレーション baseline は別テーブル/別 model_type で扱う旨を comment に明記。 |

### 任務 B — Cycle 1/2 解決 HIGH の回帰走査

**回帰なし。** 第3再計画（commit 9eeb907）は Cycle 1/2/3 で固定された全 HIGH の実行可能契約を保持:

- **DDL 11カラム PK + 3 CHECK**（HIGH#1・04-01）: action step 5・acceptance・threat T-04-03・Artifacts 全て `11カラム PK (model_type/model_version/feature_snapshot_id/as_of_datetime + 7カラム RACE_KEY)` + `CHECK (p_fukusho_hit ∈ [0,1])` + `CHECK (model_type IN ('lightgbm','catboost','logreg'))` + `CHECK (calib_method IN ('isotonic','sigmoid'))` を維持。NEW-L2 解消で D-12 comment と DDL CHECK が一致。
- **CatBoost `_code` cat_features**（HIGH#5/#6/#7・04-03）: `HIGH_CARD_CODE_COLS` を `astype(str)` で文字列化し cat_features に含めることが action step 4 / test `test_catboost_has_time` / threat T-04-13b で維持。SC#3 leak diagnostic（低基数 RARE_X + 高基数 `_code` train-only/test-unseen + `_build_intentional_leak_control` DEMONSTRABLY fail）も維持。
- **NEW HIGH-1 pred_proba injection**（Cycle 2 HIGH#14 回帰閉塞・04-04/04-05）: `predict_p_fukusho(..., pred_proba=None)` signature（04-04 action step 4）→ orchestrator call site `pred_proba=pred_proba` 明示渡し（04-05 action step 2 予測ブロック）→ `test_predict_uses_injected_pred_proba` + `test_catboost_pred_proba_injection`（np.array_equal + RuntimeError）→ threat T-04-25c/T-04-31b 連動 → verify acceptance（grep `pred_proba=` 含有）までエンドツーエンド閉塞を維持。
- **NEW-2 align_predictions guard**（04-03）: 5条件 RuntimeError guard（is_unique ×2・set 等価・len 等価・isna なし）+ `test_catboost_predict_preserves_row_order` の部分集合/重複/長不一致ケースの fail-loud 実証を維持。
- **NEW-3 staging INSERT 列明示**（04-04）: action Step 9 が `INSERT INTO prediction.fukusho_prediction ({cols_sql}) SELECT {cols_sql} FROM ..._staging`（PREDICTION_COLUMNS csv）を維持・verify grep `SELECT \* FROM prediction.fukusho_prediction_staging` == 0。
- **NEW-5 + residual #13**（04-02/04-05）: `load_native_artifact` 3ステップ真正再構築（Cycle 4 で NEW-M1/NEW-L1 強化）+ `train_and_predict(feature_df)` が label-joined frame のみ受け取り orchestrator は label join を再実行しない契約（`grep -c 'load_labels\|readonly_cur' src/model/orchestrator.py == 0`・run script 側二重防御）を維持。
- **HIGH#7 bit-identical**（04-05）: `num_threads=1`（LightGBM）/ `thread_count=1`（CatBoost）+ 固定 `as_of_datetime`（FIXED_REPRODUCE_TS）+ `np.array_equal` を維持。
- **HIGH#3/#8/#10**（04-06）: SC#3 対抗的構造診断（live-data 証明と称さない）・SC#2 比較表生成と AI 付加価値の2要素分離・KEIBA_SKIP_DB_TESTS unset で critical skipped count == 0 を維持。
...
1. **np.allclose（NEW-M1 roundtrip）vs np.array_equal（SC#4 reproduce）の整合**: 2つの assertion は異なるテスト対象を検証しており矛盾しない。SC#4（04-05/04-06）は同一プロセス内で seed+thread+as_of_datetime 固定の2回 train_and_predict で bit-identical を要求→ `np.array_equal` が正しい（固定条件下の再現性には bit-identical が必須）。NEW-M1（04-02）は save → disk unload → reload → 再構築を跨ぐ roundtrip→ joblib 直列化復元後の CalibratedClassifierCV 内部表現（`_calibrators` 注入）で微小 float 差が生じうるため `np.allclose(rtol=1e-12, atol=1e-12)` が合理的。閾値は極めて厳格（1e-12）で実質的に bit-identical に近い。両者は適切に使い分けられており・SC#4 の bit-identical 聖域を損なわない。INFO。

2. **DDL 列数表記揺れ（11 vs 9）**: 04-04 read_first 行104 に `9カラム PK` という古い記述が残る（action/acceptance/test/threat は全て `11カラム` で正しい）。Cycle 3 で「実行時影響なし・実装者は acceptance/本文が権威」と判定済みの INFO レベル。Cycle 4 でも未修正だが・実装契約の中心（action step 5・`_assert_valid_prediction_df` 行121・checksum 行172・acceptance 行191・threat T-04-23）は全て 11カラムで一貫しており・execute-phase が誤って 9カラム PK を実装する経路は存在しない。INFO（actionable ではない）。

3. **sklearn 私有 API（`_calibrators`/`calibrated_classifiers_`）の実装可能性**: 04-02 action step 5(c) は sklearn 1.9 の私有/準私有構造への依存を認めるが・NEW-M1 で scikit-learn==1.9.0 pin + `np.allclose(rtol=1e-12)` roundtrip test を安定性保証とし・pin 破壊で即時 RED で検出する閉じた契約に変換済み。pyproject.toml 行16 の pin 実在を確認。実装時に `_calibrators` 直接注入が困難な場合は `CalibratedClassifierCV(estimator=base)` 再構築 + 公開 API 範囲での再 bind にフォールバックできる余地が docstring に残されている（「`_calibrators` 属性への注入・又は `.calibrated_classifiers_` への再 bind」）。実装難度は残るが・契約は十分（roundtrip test が最終保証）。Cycle 3 と同じ判定・actionable ではない。

4. **pred_proba / as_of_datetime / model_version scoping の連動**: orchestrator（04-05）が CatBoost 予測 → align_predictions 復元 → `predict_p_fukusho(pred_proba=pred_proba)` 注入 → provenance DataFrame 構築 → `load_predictions` 1 model_type+model_version 単位呼出の全経路が action/acceptance/test で一貫。as_of_datetime は reproduce では FIXED・通常は now() で・hash は予測値のみ scope（threat T-04-25b）。連動する regression の隙間なし。

5. **CatBoost `_code` の学習+予測両経路**: 学習時（`_prepare_catboost_pool` で `astype(str)` + cat_features）は action/test で固定。予測時は orchestrator が `calib_result.calibrated.predict_proba(X_test_cb_pool)` を呼び X_test_cb_pool が sort 済み Pool の場合・その Pool 構築時にも同じ `_prepare_catboost_pool` が適用され `_code` が文字列化される経路が整合的に動く。ただし orchestrator action step 2 予測ブロックの記述は `X_test_cb_pool` をどこで構築するかを完全には明示しない（04-03 `_prepare_catboost_pool` を再利用する暗黙）。これは cycle-1 HIGH#2/NEW HIGH-1 で閉塞済の行整列契約の範囲内で・predict_p_fukusho の `pred_proba` 注入で最終 DataFrame まで伝播するため silent failure は構造的に不可能。INFO。

6. **5-wave / 6-plan 構造と MODL-01/02/03 カバレッジ**: 第3再計画は 5-wave 構造（01→02→{03,04}→05→06）を維持・files_modified 衝突なし・requirements（MODL-01: 01/02/04・MODL-02: 03/04/05・MODL-03: 02/03/05）のカバレッジも維持。Cycle 4 は文書修正中心で構造を変えていない。INFO。

### 残存 HIGH 懸念（Cycle 4 終了時点）

**なし。** Cycle 1/2/3 で閉塞した全 HIGH（Cycle 1 の19件・Cycle 2 の NEW HIGH-1・NEW-2/3/5・residual #13）は第3再計画で回帰なく保持され・Cycle 3 の4件 actionable は全て実行可能契約に変換された。リークのない馬ごとの `p_fukusho_hit` と race_id 時系列バックテストという Phase 4 核心を止める HIGH は存在しない。

### 残存 actionable MEDIUM/LOW 懸念（Cycle 4 終了時点）

**なし。** Cycle 3 の4件（NEW-M1 MEDIUM・NEW-4 残渣 LOW・NEW-L1 LOW・NEW-L2 LOW）は全て実行可能契約（action / acceptance / test / threat / must_haves / Artifacts / docstring / pyproject.toml pin）に変換され・`/gsd-execute-phase` が実装時に見えない懸念は残っていない。任務 C で走査した対抗的観点（np.allclose vs np.array_equal・DDL 列数表記揺れ・sklearn 私有 API 実装難度等）は全て INFO レベル（cosmetic/doc-level・実行正確性に影響しない）であり actionable count から除外。

### Goal-Backward Verdict (Cycle 4)

Phase 4 の核心「リークのない馬ごとの `p_fukusho_hit`」を止める懸念は存在しない。第3再計画は Cycle 1-3 の全 HIGH を実行可能契約として保持し・Cycle 3 の4件 actionable を全て PLAN/doc の実行可能内容に変換した。対抗的再走査で新規 HIGH/actionable は発見されず・np.allclose vs np.array_equal の使い分け・DDL 表記揺れ・sklearn 私有 API 依存等は全て INFO レベル（execute-phase の実装正確性に影響しない cosmetic/doc-level）。**Phase 4 は `/gsd-execute-phase` に進める。**

### Risk Assessment (Cycle 4)

**総合リスク: LOW**（Cycle 3 と同等・据え置き）

リーク防止不変量（DDL 11カラム PK + CHECK・FEATURE_COLUMNS allowlist・CatBoost `_code` cat_features 文字列化・行整列 align_predictions guard・model_version D-10 形式・base+calibrator artifact 分離・SC#3 対抗的構造診断・SC#4 固定 thread/as_of_datetime・prediction model_version スコープ swap・KEIBA_SKIP_DB_TESTS unset 最終ゲート・SC#2 2要素分離・pred_proba injection・staging INSERT 列明示・label join 境界）は全て実行可能契約として保持され・Cycle 3 残渣4件は閉じた。残るは INFO レベルの文書/cosmetic のみで・execute-phase が実装時に詰まるものではない。

---

## Current HIGH Concerns

なし。Cycle 1/2/3 で閉塞した全 HIGH（Cycle 1 の19件・Cycle 2 の NEW HIGH-1・NEW-2/3/5・residual #13）は第3再計画（commit 9eeb907）で回帰なく保持され・Cycle 3 の4件 actionable は全て実行可能契約に変換されたことを Cycle 4 で独立検証済み。CatBoost `_code` cat_features（学習+予測両経路）・predict_p_fukusho pred_proba injection・align_predictions 5条件 guard・staging-swap model_version scoping + 列明示 INSERT・DDL 11カラム PK + CHECK・model_version D-10 形式・load_native_artifact base+calibrator 真正再構築（scikit-learn==1.9.0 pin 安定性保証）・SC#3 対抗的構造診断・SC#4 bit-identical（thread pinning + 固定 as_of_datetime）・SC#2 2要素分離・KEIBA_SKIP_DB_TESTS unset 最終ゲート・label join 境界二重防御は全てエンドツーエンドで閉塞。対抗的再走査で新規 HIGH なし・cycle-1/2 回帰なし。

## Current Actionable Non-HIGH Concerns

なし。Cycle 3 の4件（NEW-M1 MEDIUM・NEW-4 残渣 LOW・NEW-L1 LOW・NEW-L2 LOW）は全て PLAN/doc の実行可能契約（action / acceptance / test / threat / must_haves / Artifacts / docstring / pyproject.toml pin）に変換済みを検証・actionable=0 達成。任務 C で走査した対抗的観点（np.allclose vs np.array_equal の整合・DDL 11/9カラム表記揺れ・sklearn 私有 API 実装難度・CatBoost 予測時 Pool 再構築の暗黙等）は全て INFO レベル（cosmetic/doc-level・execute-phase の実装正確性に影響しない）であり actionable count から除外。
