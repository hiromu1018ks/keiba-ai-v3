...
- [x] **Phase 12: p_lower EV & Falsification Evaluation** - `p_lower` 下側信頼限界によるEV判定へ移行し、評価指標拡張（selected-only calibration / EV-decile ROI / disagreement ROI / snapshot slippage）と falsification test で market residual を統計検証する (completed 2026-06-28)
...
- [x] 12-01-PLAN.md — statsmodels 依存 + compute_p_lower_conformal_shrinkage 純粋関数 + prediction p_fukusho_hit_lower 列 DDL migration 連鎖（3ファイル・Pitfall 4・EV-01/SAFE-01）
...
- [x] 12-03-PLAN.md — src/eval/falsification.py 新規（clustered SE・market_implied 再校正・Holm・logit clipping・§11.2 聖域）+ evaluator WARN gate（§15.2 不変・D-06）+ segment_eval binning 拡張 + refund_accounting slippage（EVAL-01/EVAL-02/SAFE-01）

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 12-04-PLAN.md — scripts/run_phase12_evaluation.py 新規（run_phase11_evaluation.py 構造踏襲）+ reports/12-* + switch_recommendation（D-09）+ set_primary_model Call 0件（D-10・AST check）（EV-01/EVAL-01/EVAL-02/SAFE-01）

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 12-05-PLAN.md — 対抗的監査 test_audit_p_lower_falsification.py（5段階鋳型・SAFE-01 proxy 排除 AST + falsification leakage + set_primary_model Call 0件）+ SC#5 byte-reproducible スモーク + checkpoint:human-verify（live-DB・SAFE-01）
