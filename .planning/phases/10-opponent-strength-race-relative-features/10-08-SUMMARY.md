...
metrics:
  ...
  tasks_done: 2
  tasks_checkpoint: 1
status: partial-complete-task3-checkpoint-pending
---

# Phase 10 Plan 08: REVIEW Findings の fail-loud 化・堅牢化・live-DB 再生成検証 Summary

10-REVIEW.md の 4 Critical (CR-01〜04) + 6 Warning (WR-01〜06) のうち・ユーザー決定で採用された全対象を fail-loud 化・堅牢化し・live-DB で snapshot 再生成検証を実施する gap-closure plan。核心は core value「リーク防止」の鏡像である「silent fallback 禁止」の機械保証。Task 1/2 で実装とテストを完了し・Task 3 は checkpoint:human-verify で停止中（live-DB 操作のユーザー承認待ち）。
...
| §19.1 再現性（byte-reproducible snapshot・SHA256） | Task 3 で live-DB 再生成検証予定（checkpoint 待ち） |
...
## Task 3: live-DB snapshot 再生成検証（checkpoint:human-verify 待ち）

**状態**: ユーザー承認待ち（live-DB 操作を含むため executor 側では実行せず）

**目的**: Task 1/2 で CR-02（builder Step5c dtype 正規化 + JOIN 率 fail-loud）と WR-03（snapshot.py 動的導出）を修正した。CR-02 の dtype 正規化は・歴史的に silent NaN merge が存在した場合に merge 値を変える可能性がある（silent NaN → 実データ）。WR-03 は snapshot coerce 対象列の動的導出であり・直列化結果は不変のはずだが・live-DB で Parquet schema が変わらないことを証明する必要がある（memory feature-snapshot-regen-required）。

**検証手順** (10-08-PLAN.md Task 3 how-to-verify 1-9 参照):

1. live-DB 接続確認（KEIBA_SKIP_DB_TESTS unset・readonly role）
2. 修正前 SHA256 記憶（既存 snapshot・10-VERIFICATION.md 記載の sha256=c25ae556...）
3. snapshot 再生成（Task 1/2 修正後の builder + snapshot を使う・statement_timeout 必須）
4. SHA256 比較（修正前後で一致するか）
5. SHA256 一致の場合 → `reports/10-gap-closure/regen-verification.json` に `{"sha256_unchanged": true, "silent_nan_merge_absent": true}` 記録・SC#3 byte-reproducibility GREEN・registry parity GREEN・Phase 11 入力整合確認
6. SHA256 変更の場合 → SC#5 非劣化 gate 再実行（D-16 許容幅内であること）・CR-04 RuntimeError で process が停止した場合の分岐（phase10_frame rolling 系 feature 伝播/snapshot 再生成/FEATURE_COLUMNS 再導出の問題）を鑑別
7. §12.4 metadata 確認（feature_count=106・schema_version=0.6.0・feature_cutoff_rule='race_date - 1 day'・dataset_version v1.0.0）
8. SAFE-01 聖域回帰確認（再生成 snapshot で odds/ninki proxy 0 件）
9. Phase 11 入力整合サマリ記録

**resume-signal**: Type "approved"（SHA256 一致・または SHA256 変更時に SC#5 gate_pass=True・または Phase 11 入力 snapshot 整合確認の場合）または問題を記述（SHA256 不一致 + gate_pass=False・feature ノイズ化疑い・ArrowTypeError・feature count 不一致・誤 metadata・CR-04 RuntimeError で process が停止した場合 等）
...
- §19.1 byte-reproducibility: Task 3 live-DB で検証予定
