...
provides:
  ...
  - tests/features/test_field_strength.py 性能テスト拡張（W-3 cProfile + CYCLE-3 MEDIUM #3 production-scale smoke + W-3 wall time 回帰ガード〔根拠再確認版 2026-06-27 option-a・per-race ≤0.30s + 準二次スケーリング〕）
affects:
...
decisions:
  ...
  - W-3 縮小版 5.0 秒絶対閾値は PLAN 01 設計（CYCLE-2 HIGH-C2-1 per-source-race full-pipeline 再計算・core value 必須）と構造的に両立しなかった（実測 190.5 秒 @1000 race）。**2026-06-27 ユーザー决定 option-a で根拠再設定により解決**: W-3 聖域（後追い緩和禁止）の『PLAN 更新（閾値根拠再確認）→ 再実行』手順を経て・絶対閾値を per-source-race 線形予算(≤0.30s/race・必須再計算 ~0.19s + margin) + 準二次スケーリングガード(ratio < (N_full/N_small)^2.0・H² 爆発検出・実測指数 ~1.77) に再設定（緩和でなく根拠の再設定）。W-3 核心（cProfile Python ループ無し）と production smoke が原理的証拠・本テストは回帰ガード。PLAN 01/07 の W-3 記載も同期更新・KEIBA_RUN_PERF_TESTS=1 で GREEN 実証済み。
metrics:
  duration: 55min
  completed: 2026-06-27
  tasks: 2
  files: 3
  tests: 7 audit unit tests GREEN + 3 perf tests GREEN (KEIBA_RUN_PERF_TESTS=1・W-3 回帰ガード含む根拠再設定版)
status: complete
---
...
### W-3 縮小版 5.0 秒閾値 — RESOLVED（2026-06-27・ユーザー决定 option-a・根拠再設定）

W-3 縮小版 5.0 秒絶対閾値は PLAN 01 設計（CYCLE-2 HIGH-C2-1 per-source-race full-pipeline 再計算・core value 必須）と構造的に両立しなかった（実測 190.5 秒 @1000 race・per-race 0.19s・スケーリング指数 ~1.77）。**2026-06-27 ユーザー决定 option-a により解決**。

**採用した根拠再設定（緩和でなく）**: W-3 聖域（後追い緩和禁止）の『PLAN 更新（閾値根拠再確認）→ 再実行』手順（PLAN 07 L188）に従い・絶対閾値を以下の2本に再設定した:
- **per-source-race 線形予算**: `PERF_BUDGET_SEC_PER_SOURCE_RACE = 0.30`（必須 full-pipeline 再計算 ~0.19s/race + ~57% margin）
- **準二次スケーリングガード**: `PERF_SCALING_MAX_EXPONENT = 2.0`・`elapsed_full / elapsed_small < (1000/200)^2.0 = 25`（H²=2.0 乗未満で Pitfall 2 の H² 積爆発を検出・実測 17.3× < 25 で余裕）

**W-3 証拠の3層分解**: (1) cProfile 上位3位に Python ループ無し〔test_no_python_loop_hot_spot・原理的証拠・GREEN〕(2) production-scale smoke ≤8GB/≤300s + batch 線形スケール〔test_production_scale_smoke_no_h_squared_blowup・原理的証拠・GREEN〕(3) per-race 予算 + 準二次スケーリング〔test_compute_field_strength_profile_performance・回帰ガード・GREEN・KEIBA_RUN_PERF_TESTS=1・235.6s〕。

**非採用の選択肢**: (b) PLAN 01 設計見直し（キャッシュ/増分計算）は CYCLE-2 HIGH-C2-1 値の不変性（core value）損なうリスクで却下。(c) テスト削除は W-3 聖域抵触リスクで却下。(a) が根拠再確認手順を踏むため聖域遵守として最も妥当。

**同期更新**: PLAN 01 (T-10-04)・PLAN 07 (frontmatter truth・objective・behavior・action・acceptance・done・threat・constants) の W-3 記載を根拠再設定版に更新。本 Deferred Issue はクローズ。
