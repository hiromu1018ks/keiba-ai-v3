...
Streamlit 既定フォント（system sans-serif）を使用。日本語表示は OS 既定（個人の Mac 環境）。明示的に宣言するのは Streamlit テーマで上書き可能な4役割のみ。

| Role | Size | Weight | Line Height |
|------|------|--------|-------------|
| Body | 14px（Streamlit 既定） | regular (400) | 1.5 |
| Label | 13px（サイドバーウィジェットラベル・`st.caption`） | medium (500) | 1.4 |
| Heading | 20px（`st.subheader`・タブタイトル・h2 相当） | semibold (600) | 1.25 |
| Display | 28px（`st.title`・単一・ページ先頭のみ） | bold (700) | 1.2 |
