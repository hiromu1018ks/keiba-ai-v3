...
metrics:
  ...
  files_created: 5
  files_modified: 1
  tests_passed: 51 passed / 0 skipped (Plan 01+02+03 全 green・skip 0・LOW-1 緩和基準が Plan 03 で本格化)
...
## Task 3: checkpoint:human-verify（本 SUMMARY 後に人間が承認）

Task 3 は `checkpoint:human-verify`（gate="blocking"）で・本 plan の完了後も自動で解決しない。Pre-checkpoint 自動ゲート（tests/ui/ 51 green + import smoke + 両 CLI --help）は全て合格済み・Task 3 は人間がブラウザ（http://localhost:8501）で `uv run streamlit run src/ui/app.py` を起動し・UI-01（予測一覧・スタンプ inline・ダウンロード）・OUT-01/OUT-02 CSV・D-05 Plotly・honest 注記・§16.1 除外項目を確認して approved を返すことを待つ。

resume-signal: "approved"（全項目確認OK）または課題を記述（修正後再確認）。
