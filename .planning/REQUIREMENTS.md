...
- [ ] **EV-01**: `p_fukusho_hit` と複勝オッズ下限/上限から `EV_lower = p × odds_lower` / `EV_upper = p × odds_upper` を算出できる
- [ ] **EV-02**: EV・確率・オッズ下限のみで推奨ランク（S/EV≥1.20, A/EV≥1.10, B/EV≥1.05, C/EV≥1.00, D/その他）を初期定義どおりに算出できる（未定義の予測信頼度は使わない）
