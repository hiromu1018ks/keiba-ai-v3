...
"""
# ruff: noqa: E501  (本ファイルは SQL/DDL リテラル・長い docstring を保持するため行長は緩和)
...
def make_model_version(feature_snapshot_id: str, model_type: str, version_n: int = 1) -> str:
...
def predict_p_fukusho(
    ...
    **Cycle 2 NEW HIGH-1 (pred_proba 注入):**
    ...
    if missing:
        raise ValueError(f"predict_p_fukusho: race_df missing required columns: {missing}")
    if not race_df.index.equals(X.index):
        raise ValueError("predict_p_fukusho: race_df.index does not match X.index")
