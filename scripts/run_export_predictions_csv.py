...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # 分離できるようにする。CR-02 で修正した cwd 依存 (FileNotFoundError) 経路もここで捕捉される。
        # sys.exit(main()) で traceback 表示 → exit 1 になる経路も回避。
        logger.error("OUT-01 出力失敗（DB 以外のエラー）: %s", exc, exc_info=True)
        return 2
